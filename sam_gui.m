...
function drawGUI()
    ...
    uiObj.feMethod = uicontrol(uiObj.panelParams, ...
            ...
            'FontWeight', 'normal', ...
            'Units', 'pixels', ...
            'Position', [180, hPanel-120, 140, 25]);
    ...
    uiObj.feCompLbl = uicontrol(uiObj.panelParams, ...
            ...
            'FontWeight', 'normal', ...
            'Units', 'pixels', ...
            'Position', [330, hPanel-120, 110, 20]);
    ...
    uiObj.feComp = uicontrol(uiObj.panelParams, ...
            ...
            'FontWeight', 'normal', ...
            'Units', 'pixels', ...
            'Position', [450, hPanel-120, 130, 25]);
...
function selectFiles(h, classNum)
    [files, path] = uigetfile([pwd filesep '*.nii'], ...
                    'Select Class 0 images','MultiSelect', 'on');
                
    if ~isequal(files, 0)
        appData = guidata(h);
        hList = eval(['appData.uiObj.class' classNum 'Files']);
        set(hList, 'String', files);
        hText = eval(['appData.uiObj.class' classNum 'NumFiles']);
        set(hText, 'String', [num2str(numel(files)) ' files selected']);
        
        appData.(['class' classNum]).files = files;
        appData.(['class' classNum]).path = path;
        guidata(h, appData);
    end
end

function selectAtlas(h)
    [file, path] = uigetfile(['./atlas/*.nii'],'Select atlas image');
    if ~isequal(file, 0)
        appData = guidata(h);
        hEdit = appData.uiObj.atlasFile;
        set(hEdit, 'String', fullfile(path, file));
        appData.atlas.file = file;
        appData.atlas.path = path;
        guidata(h, appData);
    end
end

function selectOutput(h)
    [file, path] = uiputfile(['./*.nii'],'Select output file');
    if ~isequal(file, 0)
        appData = guidata(h);
        hEdit = appData.uiObj.outputFile;
        set(hEdit, 'String', fullfile(path, file));
        appData.output.file = file;
        appData.output.path = path;
        guidata(h, appData);
    end
end
...
function start(h)
    ...
    % Check atlas is defined
    if ~isfield(appData, 'atlas')
        msgbox('Please select an atlas image')
        return
    end
    
    % Check atlas is defined
    if ~isfield(appData, 'output')
        msgbox('Please indicate an output file name')
        return
    end
